<template>
  <div class="w-full xl:w-2/12 flex justify-between xl:justify-end items-center gap-x-2">
    <button @click.stop="changelocal()" type="button" class="hidden lg:flex items-center font-bold justify-center px-4 py-2 rounded-lg cursor-pointer text-fpDark1 dark:text-slate-50 hover:dark:text-white hover:text-fp1 relative z-50 transition align-middle text-sm">
      <Icon :name="locale == 'ar'
    ...
    <button title="Dark mode and Light mode" :class="auth.isLoggedIn ? '' : 'ms-auto'"
      @click.stop="globalStore.toggleDarkmode()" class="btn_actions_header">
    ...
    <button :title="t('logout')" type="button" @click="auth.logout()"
      class="btn_actions_header grid place-content-center" v-if="auth.isLoggedIn">
