<template>
  <section class="pb-8 lg:pb-24">
    ...
    <div class="container mx-auto mt-10">
      <div class="">
        <div class="grid lg:grid-cols-5 grid-cols-1 gap-4 items-center text-center lg:text-start">
          <div class="lg:col-span-1 text-center">
            <Skeleton type="image" v-if="Object.keys(teacher).length == 0" />
            <div class="w-52 h-52 mx-auto rounded-full overflow-hidden shadow-lg">
      ...
      <TransitionGroup name="taps-down">
        <div class="mt-10" v-show="!showSection">
          <h2 class="text_clip text-xl lg:text-4xl font-extrabold mb-10">{{ $t("classes") }}</h2>
        ...
        <div class="mt-10" v-show="showSection">
          <div class="flex items-center justify-between mb-10">
            <h2 class="text_clip text-xl lg:text-4xl font-extrabold">{{
              $t("courses") }}</h2>
            <button @click="showSection = false" class="custom_btn1 !m-0 !text-sm lg:!text-lg">
              {{ $t("all_courses") }}
        ...
        <div class="text-center" v-show="courses.length == 0 && showSection">
          ...
          <div class="mt-10">
            <h2 class="lg:text-3xl text-xl text-center text-black dark:text-white font-semibold leading-relaxed pb-1">
              {{ $t("not_found_courses") }}
            </h2>
          </div>
        </div>
